...
with st.sidebar:
    ...
    with st.expander("💼 Capital & Base Fees", expanded=True):
    ...
    with st.expander("🎯 Performance Structure", expanded=True):
...
def get_cagr(g_list, m, p):
    df_temp = run_full_simulation(g_list, m, p)
    res = df_temp["Net Closing"].iloc[-1]
    return ((res / initial_value)**(1/len(g_list)) - 1) * 100

c1, c2, c3 = st.columns(3)
with c1:
    st.caption("Growth Sensitivity")
    s_g = pd.DataFrame({"Shift": [f"{s*100:+.1f}%" for s in shifts],
                        "CAGR": [get_cagr([x+s for x in growth_rates], base_mgmt, base_perf) for s in shifts]})
    st.table(s_g.style.format(precision=2))
with c2:
    st.caption("Mgmt Fee Sensitivity")
    s_m = pd.DataFrame({"Shift": [f"{s*100:+.1f}%" for s in shifts],
                        "CAGR": [get_cagr(growth_rates, base_mgmt+s, base_perf) for s in shifts]})
    st.table(s_m.style.format(precision=2))
with c3:
    st.caption("Perf Fee Sensitivity")
    s_p = pd.DataFrame({"Shift": [f"{s*100:+.1f}%" for s in shifts],
                        "CAGR": [get_cagr(growth_rates, base_mgmt, base_perf+s) for s in shifts]})
    st.table(s_p.style.format(precision=2))

st.write("#### 3-Way Stress Test: Growth vs. Performance Fee")
matrix = [[get_cagr([g+sg for g in growth_rates], base_mgmt, base_perf+sp) for sp in shifts] for sg in shifts]
