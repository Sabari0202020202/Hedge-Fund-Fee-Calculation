...
with st.sidebar:
    ...
    with st.expander("🎯 Performance Incentive", expanded=True):
    ...
    with st.expander("🔬 Sensitivity Range"):
...
with st.container():
    ...
    with c1:
        if st.button("➕ Add Year"): st.session_state.years_count += 1
    with c2:
        if st.button("➖ Remove Year") and st.session_state.years_count > 1: st.session_state.years_count -= 1
...
# --- MAIN TABLE ---
st.write("### 📄 Annual Performance Ledger")
...
# --- SENSITIVITY ---
st.markdown("---")
st.write("### 🎯 Strategic Sensitivity Analysis")
